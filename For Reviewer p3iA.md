...
- **Unified the excessive communication methods:** Existing Multi-Agent System(MAS) frameworks have too many communication methods, typically designed for specific tasks (citation). The core of multi-agent communication lies in efficiency, comprehensiveness, and flexibility (citation). RelCom possesses these characteristics and is easy to implement, making it suitable for universal and general experimental research (Section 3.1).
...
- **For the normal node**, the safety of the individual LLM makes it difficult to attack or influence, which in turn increases the likelihood of the attacker being corrected during communication (aggregation safety). We provide results below:
...
- **Extra Experiments on Other LLMs**: We have added experiments for different LLMs *(both open and closed-source LLMs)*. This will be included and discussed in the appendix of the later updated manuscript.
...
- **For Bias and Harmful information:** RQ2 demonstrates that topologies with higher connectivity lead to higher potential to correct attacker nodes which may not be a common observation, as demonstrate below:
...
- **For GPT-4o-mini:** GPT-4o-mini and GPT-4o incorporate the same security mechanisms and maintain high performance. [1]
- **For GPT-3.5-turbo:** GPT-3.5-turbo has weaker security [2], making it easier to observe the impact of bias and harmful information on MAS in different topologies.
